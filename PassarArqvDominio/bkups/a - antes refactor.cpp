...
int main() {
    ...
    //VERIFICANDO SE EXISTE ALGUM ARQUIVO JA ATUALIZADO
    jaAttLoc = versaoNova + "jaAtualizado.txt";
    std::ifstream jaAtualizado(jaAttLoc);
    std::string jaLinha;
    std::vector<std::string> jaArqs;

    if (!jaAtualizado.is_open() && fs::exists(jaAttLoc)) {
        std::cerr << "Erro ao abrir o arquivo jaAtualizado.txt" << std::endl;
    }
    while (std::getline(jaAtualizado, jaLinha)) {
        jaArqs.push_back(jaLinha);
    }
    jaAtualizado.close();
    ///////////////////////
    ...
    for (const std::string& s : arqs) {
        ...
        try {
            ...
            if (!fs::exists(destino)) {
                ...
                gravaJaAtt(jaAttLoc, nomeArquivo);
            }
            else {
